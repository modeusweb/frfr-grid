...
- **Точный интерактив** — hit-test по реальным границам клеток: выделение, drag и растягивание корректны при любых треках (`1fr 3fr 1fr`, `100px 1fr`, …)
- **Grid areas** — создание, переименование, дублирование и удаление областей; быстрое переименование двойным кликом по области
- **Пилюли-подписи** — названия областей в виде аккуратных бейджей с обрезкой длинных имён
- **Растягивание** областей за круглую ручку с умным ограничением по занятым ячейкам и превью нового размера
- **Перетаскивание** областей на свободные места с зелёной/красной индикацией валидности
- **Размеры треков** — `fr`, `px`, `%`, `rem`, `em`, `auto`, `min-content`, `max-content`; быстрые пресеты `1fr` / `2fr` / `100px`
...
- **Шаблоны** — 8 готовых лейаутов: sidebar + content, header/main/footer, holy grail, блог, карточки
- **Предпросмотр** — чистая сетка без служебных подсказок
- **Поделиться** — конфигурация сериализуется в URL
- **Undo / Redo** — история на 100 шагов с горячими клавишами
...
- **Попапы** — закрываются кликом по фону или `Escape`, скроллятся при нехватке высоты
...
- **Исходный код** — футер со ссылкой на репозиторий GitHub
...
```
app/
  layout.tsx            # метаданные, верификация поисковиков, тема, JSON-LD
  page.tsx              # единственная страница с генератором
  globals.css           # Tailwind 4, @theme, анимации, утилиты
  not-found.tsx         # страница 404
  robots.ts             # /robots.txt
  sitemap.ts            # /sitemap.xml
  manifest.ts           # /manifest.webmanifest (PWA-манифест)
components/grid-generator/
  GridGenerator.tsx     # корневой компонент: состояние, история, обработчики
  GridEditor.tsx        # интерактивная сетка: hit-test, выделение, resize, move
  GridSettings.tsx      # панель настроек (колонки, строки, gap, треки)
